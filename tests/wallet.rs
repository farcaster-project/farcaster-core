...
fn key_manager_and_recover_secret() {
    ...
    let sig = key_manager
        .sign_with_key(ArbitratingKeyId::Buy, bytes)
    ...
    let adaptor_sig = key_manager
        .adaptor_sign_with_key(ArbitratingKeyId::Buy, &btc_adaptor_pubkey, bytes)
        .unwrap();
    assert!(key_manager
        .verify_adaptor_signature(&buy_pubkey, &btc_adaptor_pubkey, bytes, &adaptor_sig)
        .is_ok());
    let adapted_sig = key_manager
        .adapt_signature(AccordantKeyId::Spend, adaptor_sig.clone())
        .unwrap();
